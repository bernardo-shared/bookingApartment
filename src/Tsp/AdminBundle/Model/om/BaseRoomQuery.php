...
abstract class BaseRoomQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'default', $modelName = 'Tsp\\AdminBundle\\Model\\Room', $modelAlias = null)
    {
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Tsp\AdminBundle\Model\FlatQuery A secondary query class using the current class as primary query
     */
    public function useFlatQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinFlat($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Flat', '\Tsp\AdminBundle\Model\FlatQuery');
    }
